...
async def repeater(app: Ariadne, message: MessageChain, group: Group):
    ...
    if group_id in group_repeat.keys():
        ...
        if group_repeat[group.id]["lastMsg"] != group_repeat[group.id]["thisMsg"]:
            ...
            if group_repeat[group.id]["thisMsg"] != group_repeat[group.id]["stopMsg"] and group_repeat[group.id]["stopMsg"]=='':
                group_repeat[group.id]["stopMsg"] = group_repeat[group.id]["thisMsg"]
                try:
                    await app.send_group_message(group, message.as_sendable())
                except AccountMuted:
                    pass
            elif group_repeat[group.id]["thisMsg"] == group_repeat[group.id]["stopMsg"] and group_repeat[group.id]["lastMsg"] == group_repeat[group.id]["stopMsg"]:
                try:
                    await app.send_group_message(group, message.as_sendable())
                except AccountMuted:
                    pass
